...
def write_iterations(phases, startcol):
...
def write_charts(iterations):
    workbook = writer.book
    chart = workbook.add_chart({'type': 'line'})
    for idx, i in enumerate(iterations):
        i.to_excel(writer, sheet_name='Графики', startcol=idx*2)
        chart.add_series({'values': ["Графики", 1, idx*2+1, 1+i.size, idx*2+1]})
    writer.sheets["Графики"].insert_chart('F3', chart)
...
floor = np.float64(0.5)
phases = split_by_floor(phases, floor)
phases.name = "Итерация 1"
charts_phases.append(phases.droplevel(0))
iteration1 = count_stats(phases, 1)
write_iterations(phases, 4)


floor = np.float64(0.8)
phases = split_by_floor(phases, floor)
phases.name = "Итерация 2"
charts_phases.append(phases.droplevel(0))
iteration2 = count_stats(phases, 2)
write_iterations(phases, 5)

write_charts(charts_phases)
